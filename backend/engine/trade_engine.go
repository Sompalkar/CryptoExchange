...
type TradeEngine struct {
	...
	marketMakers map[string]*MarketMakerBot
...
func NewMarketMakerBot(engine *TradeEngine, orderBook *models.OrderBook, config MarketMakerConfig, pair string) *MarketMakerBot {
	return &MarketMakerBot{
		engine:    engine,
		orderBook: orderBook,
		config:    config,
...
func (bot *MarketMakerBot) updateOrders() {
	...
	for i := 0; i < bot.config.MaxOrders; i++ {
		buyOrder := &models.Order{
			Pair:   bot.pair,
			Type:   models.OrderTypeBuy,
			Price:  buyPrice * (1 - float64(i)*0.001),
			Amount: bot.config.Volume,
			Status: models.OrderStatusPending,
			IsBot:  true,
			BotID:  bot.botID,
		...
		sellOrder := &models.Order{
			Pair:   bot.pair,
			Type:   models.OrderTypeSell,
			Price:  sellPrice * (1 + float64(i)*0.001),
			Amount: bot.config.Volume,
			Status: models.OrderStatusPending,
			IsBot:  true,
			BotID:  bot.botID,
		...
		bot.engine.ProcessOrder(buyOrder)
		bot.engine.ProcessOrder(sellOrder)
	}
}
...
func (e *TradeEngine) createNewBotOrder(pair string) {
	...
	price := generateRandomPrice(e.orderBooks[pair])
	amount := generateRandomAmount()
...
func generateRandomPrice(book *models.OrderBook) float64 {
	bestBid := book.GetBestBid()
	bestAsk := book.GetBestAsk()
	if bestBid > 0 && bestAsk > 0 {
		spread := bestAsk - bestBid
		return bestBid + spread*rand.Float64()
	}
	return 1000 + rand.Float64()*100 // fallback price
}
